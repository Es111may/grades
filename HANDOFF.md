...
**Сделано (Phase 0–8)**: каркас, импорт Excel, auth (email+password — Keycloak отвергнут), форма оценки, портрет дизайнера, админ-матрица + грейды + гейты, канбан пользователей, роль stardiz, расширенные права лида.
...
## Что идёт следующим (Phase 9+)

См. `02_PRD.md` § 11. По приоритету:

1. **Phase 9** — 9-Box матрица потенциала (3×3 grid с drag-n-drop)
2. **Phase 10** — Канбан как основной просмотр + popup карточки 360° с 4 действиями. Удалить старый `/lead` («Мои дизайнеры»), интегрировать в `/admin/users` с фильтром «Все/Мои»
3. **Phase 11** — Лидерборд по уровням (заменит вкладку «Таблица»)
4. **Phase 12** — Визуальная доводка формы оценки
5. **Phase 13** — Разделение `Skill.description` и `Skill.confirmationGuide`
6. **Phase 14** — Самооценка дизайнера + загрузка подтверждений (SkillEvidence + SelfAssessment)
7. **Phase 15** — График скорости роста в popup карточки
