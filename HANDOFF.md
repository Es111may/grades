...
## ⚠️ САМОЕ АКТУАЛЬНОЕ (10.07.2026, версия 0.34.1)

Ниже по файлу состояние на май — оно устарело, но полезно для истории.
Свежее:

- **Тёмная Apple-тема + Onest** (вместо светлой + Aeonik). Палитра на
  CSS-переменных (`globals.css`: `:root` тёмная, `[data-theme='light']`
  светлая), **тумблер тем в шапке** (`ThemeToggle`, `src/lib/theme.ts`).
  Цифры пропорциональные (tnum off — Pavel просил компактнее). Логотип —
  два PNG (светлый/тёмный) через классы `.light-only`/`.dark-only`.
- **Закрыты фазы**: 15 (спарклайн XP в карточке 360), 16 полностью
  (перформанс ClickHouse + композит 40/40/20 c 9-Box), 17 (ИПР),
  19 (аудит), методика 9-Box в аккордеоне матрицы + описания в ячейках.
- **Следующая большая работа — редизайн по концептам**:
  `design-concepts/README.md` — согласованные концепты «Команда» и
  «Портрет» + полный чек-лист переноса + принятые решения.
  Артефакты и HTML лежат там же.
- **Откат по визуалу**: тег `v0.34.1-pre-concept-redesign` (состояние
  прода до переноса концептов). Ранний тег: `v0.24.1-pre-portrait-redesign`.
- **Railway**: прод — проект `selfless-inspiration` (IDA), домен
  `grades-production-1547.up.railway.app`. Дубль `zealous-patience`
  (`...-d9b2`, отдельная БД!) удалён 29.06 — из-за него лид публиковал
  оценки «в никуда». Диагностика: `/api/admin/diag-leaderboard` (admin).
