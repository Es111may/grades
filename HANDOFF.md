...
7. **2 теста в `grade.test.ts` красные — так и задумано, НЕ чинить вслепую.**
   Разобрано 29.07.2026, Pavel решил оставить как есть.
   - *«250 XP без обязательного навыка → грейд ниже»* (ждёт `middle`, код даёт
     `senior`). `calcGrade` проверяет гейты **только рассматриваемого грейда**,
     а не всех нижних. В матрице гейты не накопительные: джун+ требует
     «Компоненты и лейауты», мидл+ — «Защита», синьор — «Проактивность».
     Поэтому дизайнер с 250 XP и нулём в автолейаутах получит синьора, хотя в
     листе «Гейты (билды)» прямо написано «Без автолейаутов дальше не пускаем».
     Дырка в антифарме реальна, но практически недостижима (нужен профиль
     «сильные переговоры при нуле в базовой технике»), поэтому отложена.
     Починка = сделать гейты накопительными; она может понизить расчётный
     грейд живым людям, `gradeFloor` прикроет только тех, кому грейд зафиксирован.
   - *«162 XP с непройденным гейтом «Мидл» → Пре-мидл»* — тест опирается на
     `skillId: 999`, которого нет в фикстуре. Такие гейты код игнорирует
     осознанно (`onlyActiveGates`) — защита от «непройденного навыка #546»
     после удаления навыка из матрицы. Тест надо переписать на реальный навык,
     независимо от решения по накопительным гейтам.
