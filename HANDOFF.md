...
**Версия в `package.json`:** 0.66.0 (Phase 23.2 — планирование грейдирования: дата, статусы, контроль просрочек).
...
11. **Phase 23 — HR-контур.** План: `design-concepts/phase-23-hr-contour.md`.
    Перенос таблицы «Ида Дизайн Матрица дизов» в сервис: реестр всех людей
    (включая ушедших, БА / UX-редакторов / исследователей), компенсации,
    экономика, вилки.
    **Главное открытие 29.07.2026:** почти всё уже лежит в ClickHouse-копии
    HR-портала (`hr_portal_current`, 41 таблица) — ставки, история изменений
    з/п, даты найма и увольнения, причины ухода с признаком инициатора
    (сотрудник / компания), отделы и команды. Сервис туда уже ходит —
    `/api/users/[id]/last-raise`. Поэтому это **сверка, а не синк**: врут оба
    источника, разбор конкретных расхождений — в плане.
    - **23.2 — планирование грейдирования: ЗАКРЫТА 29.07.2026 (0.66.0).**
      Дата вписывается руками, к сезону не привязана (Pavel: сезоны
      «примерно май и октябрь-ноябрь», но смещаются). «Проведено» не
      хранится флагом, а выводится из `Assessment.publishedAt >=
      nextGradingSetAt` — отметка постановки нужна, чтобы досрочное
      грейдирование не читалось как просрочка. Логика —
      `src/lib/gradingPlan.ts` (19 тестов), чип —
      `src/components/GradingPlanChip.tsx`, порог просрочки 14 дней.
      Переносы даты сознательно НЕ отслеживаем (решение Pavel).
      Колонка в лидерборде показывает только дизайнеров (`role === 'designer'`) —
      стардизы видны в фиде «Требует внимания» и в поп-апе 360.
    - 23.1 Реестр · 23.3 Сверка с HR · 23.4 Компенсации · 23.5 Досев из
      таблицы · 23.6 Экономика — не начаты. Открытые вопросы — в конце плана.
12. **Phase 24 — накопительные гейты (подтверждённый баг, Pavel 29.07.2026).**
    ...
    не смотрит гейты нижних. В матрице гейты не накопительные — каждый грейд
    добавляет свои навыки: джун+ → «Компоненты и лейауты», мидл →
    «Эстимирование», мидл+ → «Защита», синьор → «Проактивность» (лист «Гейты
    (билды)» в `Шаблон Скиллсет 2.0.xlsx`; пре-мидла там нет вовсе).
    **Следствие:** дизайнер с 250 XP и нулём в автолейаутах получит синьора,
    если прошёл гейты синьора, — хотя в матрице про автолейауты прямо написано
    «Критично! Без автолейаутов дальше не пускаем». Антифарм через гейты не
    работает.
    - **Починка:** грейд достигнут, только если пройдены его гейты И гейты всех
      грейдов ниже. Заодно корректно закрывает пре-мидл — своих гейтов нет, но
      джун-плюсовые унаследует.
    - **Риск:** правка понижает расчётный грейд живым людям. `gradeFloor`
      прикрывает только тех, кому грейд зафиксирован явно. Перед применением
      стоит собрать отчёт «кого затронет» (скрипт должен идти внутри Railway —
      `postgres.railway.internal` снаружи недоступен).
    - **Тесты:** «250 XP без обязательного навыка → грейд ниже» станет зелёным
      сам. А «162 XP с непройденным гейтом «Мидл» → Пре-мидл» надо переписать
      независимо: он опирается на `skillId: 999`, которого нет в фикстуре, а
      гейты по отсутствующим и архивным навыкам код игнорирует осознанно
      (`onlyActiveGates` — защита от «непройденного навыка #546» после удаления
      навыка из матрицы). Проверять блокировку нужно на реальном навыке.
...
7. **2 теста в `grade.test.ts` красные — это ПОДТВЕРЖДЁННЫЙ БАГ, а не мусор в
   тестах.** Красный прогон здесь ожидаем: тесты описывают правильное
   поведение, которого в коде пока нет. Не «чинить» их подгонкой под код.
   Починка запланирована отдельной фазой — см. **Phase 24** в очереди.
   Разобрано 29.07.2026, Pavel подтвердил баг и решил отложить.
   - Пре-мидл — полугрейд для мотивации между джун+ и мидлом, своих гейтов в
     матрице у него нет, и вилка совпадает с джун+ (75–100) осознанно. Не
     считать это дефектом; при алертах «дороже вилки» пре-мидл нужно либо
     исключать, либо дать ему вилку под реальность (факт ≈ 120).
